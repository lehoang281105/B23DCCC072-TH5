...
const ClubMembers: React.FC = () => {
	...
	const [selectedClub, setSelectedClub] = useState<string | null>(null); // Lưu ID câu lạc bộ được chọn
	...
	// Hàm lấy tên câu lạc bộ dựa trên ID
	const getClubName = (clubId: string) => {
		const club = clubs.find((c) => c.id === clubId);
		return club ? club.name : 'Không xác định';
	};

	// Lọc thành viên dựa trên ID câu lạc bộ được chọn
	const filteredMembers = selectedClub
		? members.filter((member) => member.club === selectedClub && member.status === 'Approved')
